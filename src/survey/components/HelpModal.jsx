...
const CONTENT = {
    ...
    sections: [
      {
        title: '功能地圖',
        items: [
          { label: '我的評量', desc: '選擇題庫開始作答，可多次重測。若您屬於某個班別，課前／課後身份會由班別階段自動判定；未加入班別時可自行切換。' },
          { label: '送出後', desc: '成績會就地顯示在同一頁，點「查看完整分析」才會離開，確保您至少能看到這次的分數與落點。' },
          { label: '我的分析', desc: '查看最新得分、構面落點、歷次趨勢與教練評語。作答紀錄任一列都可點開查看該次的完整報告，不只最新一筆。' },
          { label: '360° 評測', desc: '評測您的主管、同儕或部屬，並查看整合多方回饋的分析報告（限領導力評量）。選好對象與關係即直接開始作答，不需再多按一次確認。' },
      ...
      },
      {
        ...
        items: [
          ...
          { label: '課前 / 課後', desc: '有加入班別時由班別目前所處的階段自動判定；未加入班別的自主重測則自行選擇，第一次為「課前」基準、最新一次為「課後」，兩者相減即為學習成長幅度。' },
  ...
  coach: {
    role: '教練',
    summary: '你可以在同一個班級工作區看報告、寫評語、管理成員與設定，並追蹤 360° 多元評測進度。',
    sections: [
      {
        title: '功能地圖',
        items: [
          { label: '班級 → 總覽', desc: '選定班別後預設看到的畫面：已填答／已寫評語人數（KPI 主指標）、班級雷達圖、構面 × 成員熱力圖（一眼看出哪個構面弱、是誰拉低的）、成員比較表（含達成率長條、與班平均差距、評語已寫／未寫），以及班級整體評語欄位。' },
          { label: '查看成員報告', desc: '在成員比較表點「查看」，會開啟報告＋評語合一的側邊抽屜：左邊是該學員完整報告（雷達圖、構面分數、其他教練的評語），右邊直接寫或編輯你的評語，並可用上一位／下一位在成員間切換，不必關掉重找。' },
          { label: '批次匯出／班級報告', desc: '總覽右上角「批次匯出個人報告」可一次列印全班每位成員的個人報告；「產出班級專業分析報告」則產出整班彙整的 PDF。' },
          { label: '班級 → 成員與設定', desc: '施測時間軸（開始／截止日期、發佈成果）、報到 QR Code、評量設定（目標人數、重點構面與訓練備注）、成員管理（勾選加入／CSV 匯出）、批量匯入名單。' },
      ...
      },
      {
        ...
        items: [
          ...
          { label: '個人評語 vs 班級評語', desc: '個人評語在報告側邊抽屜中針對單一學員撰寫，儲存在其作答紀錄上；班級評語在「總覽」底部填寫，是針對整個班別的整體觀察，學員進入分析頁時都能看到。' },
          { label: '構面 × 成員熱力圖', desc: '橫軸是構面、縱軸是成員，色塊越紅代表分數越低、越綠代表越高，用來快速定位這班哪個構面弱、是被誰拉低的。' },
      ...
      },
      {
        ...
        items: [
          ...
          { label: '如何新增班別成員？', desc: '在「成員與設定」的成員管理區塊勾選已註冊用戶，或在批量匯入區填入姓名、Email，尚未註冊者列為「待加入」；也可以直接請學員掃報到 QR Code 自行加入。' },
          { label: '如何知道哪些學員還沒寫評語？', desc: '在「總覽」的 KPI 區塊看整班「已寫評語」人數，或在成員比較表的「評語」欄位看每個人是「已寫」還是「未寫」。' },
          { label: '360° 評測角色如何分配？', desc: '由學員自行於「360° 評測」頁面選擇要評測的對象與角色關係，選好後直接進入作答，教練目前無法代為指派。' },
    ...
    sections: [
      {
        title: '功能地圖',
        items: [
          { label: '評量開關', desc: '管理後台頁首常駐區塊（不論切換到哪個分頁都看得到），可啟用或停用各評量，停用後一般用戶的首頁將不再顯示該評量。' },
          { label: '整體統計', desc: '依選取的評量顯示 KPI 指標、構面達成率雷達圖、落點等級分佈，以及可搜尋、依欄位排序的填答者明細表（可匯出 CSV）。' },
          { label: '用戶角色管理', desc: '可搜尋姓名或 Email、依欄位排序；變更用戶為「教練」或還原「一般用戶」，並可產生密碼重設連結交由用戶自行重設。' },
        ],
      },
      {
        title: '名詞解釋',
