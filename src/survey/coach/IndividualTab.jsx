          ...
          {assessmentIds.map((id) => (
            <button key={id} type="button" role="tab" aria-selected={selectedAssessmentId === id} onClick={() => setSelectedId(id)}
              className={`rounded-lg px-3 py-1.5 text-sm font-semibold transition-colors ${
                selectedAssessmentId === id ? 'bg-ink-700 text-white' : 'border border-slate-300 bg-white text-slate-600 hover:bg-slate-50'
              }`}
            >
          ...
          {rows.map(({ sub, user, myComment, otherComments }) => (
            <div key={sub.id} className="rounded-xl border border-slate-200 bg-white p-4">
              <div className="flex flex-wrap items-center gap-x-4 gap-y-1">
                ...
                <span className="font-bold text-brass-600">{sub.result?.total} / {sub.result?.maxScore}</span>
                <span
              ...
              {myComment && editingSubId !== sub.id && (
                <div className="mt-2 rounded-lg border-l-4 border-brass-400 bg-brass-50 px-3 py-2">
                  <p className="text-xs font-semibold text-brass-600">我的評語</p>
