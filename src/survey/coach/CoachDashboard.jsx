      ...
      <div className="lg:col-span-2">
        ...
        {creating && (
          <div className="mb-3 rounded-xl border border-brand-200 bg-brand-50 p-3 space-y-2">
            ...
            {newDims.length > 0 && (
              <div>
                <p className="mb-1 text-xs font-semibold text-brand-600">重點構面（選填，可複選；報告將優先呈現）</p>
          ...
          <div className="space-y-5">
            ...
            <div className="rounded-xl border border-brand-200 bg-brand-50/50 p-4">
              ...
              {(() => {
                ...
                return (
                  <>
                    <p className="mb-1.5 text-xs font-semibold text-brand-600">重點構面（可複選；報告將以 ⭐ 標記並優先呈現）</p>
                    ...
                    {focusDims.length > 0 && (
                      <div className="space-y-2">
                        <p className="text-xs font-semibold text-brand-600">重點構面內容（培訓目標 / 觀察重點）</p>
                        {focusDims.map((id) => {
...
function MultiRaterTab({ users, submissions }) {
  const [selectedUserId, setSelectedUserId] = useState(null);
  const [assessmentId, setAssessmentId] = useState('leadership-9d');

  const assessmentIds = [...new Set(submissions.map((s) => s.assessmentId ?? 'ai-competency'))];
  const filteredSubs = submissions.filter((s) => (s.assessmentId ?? 'ai-competency') === assessmentId);

  // Build progress matrix: for each non-admin user, which raterTypes have at least one submission
  const nonAdminUsers = users.filter((u) => u.role !== 'admin');
  ...
  return (
    <div>
      {assessmentIds.length > 1 && (
        <div className="mb-4 flex flex-wrap gap-2">
          {assessmentIds.map((id) => (
            <button key={id} type="button" onClick={() => setAssessmentId(id)}
              className={`rounded-lg px-3 py-1.5 text-sm font-semibold transition-colors ${
