...
describe('ICS roundtrip', () => {
  ...
  it('parses local floating DTSTART (no Z) as local time, not UTC', () => {
    // RFC 5545: a DTSTART without Z or TZID is a "floating" local time.
    // Importing it should preserve the clock time in the local timezone.
    const ics = [
      'BEGIN:VCALENDAR', 'BEGIN:VEVENT',
    ...
    const parsed = parseIcs(ics);
    expect(parsed).toHaveLength(1);
    // Local hour should be 9 (not shifted by UTC offset)
    expect(new Date(parsed[0].startAt).getHours()).toBe(9);
    expect(new Date(parsed[0].endAt).getHours()).toBe(10);
  });

  it('parses UTC DTSTART (trailing Z) as UTC', () => {
    const ics = [
      'BEGIN:VCALENDAR', 'BEGIN:VEVENT',
      'SUMMARY:UTC Meeting',
    ...
    const parsed = parseIcs(ics);
    expect(parsed).toHaveLength(1);
    expect(new Date(parsed[0].startAt).getUTCHours()).toBe(9);
    expect(new Date(parsed[0].endAt).getUTCHours()).toBe(10);
  });
});
